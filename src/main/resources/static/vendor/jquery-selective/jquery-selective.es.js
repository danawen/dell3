...
var DEFAULTS = {
  ...
  tpl: {
    frame: function() {
      return `<div class="${this.namespace}"><div class="${
        this.namespace
      }-trigger">${this.options.tpl.triggerButton.call(this)}<div class="${
        this.namespace
      }-trigger-dropdown"><div class="${
        this.namespace
      }-list-wrap">${this.options.tpl.list.call(
        this
      )}</div></div></div>${this.options.tpl.items.call(this)}</div>`;
    },
    search: function() {
      return `<input class="${
        this.namespace
    ...
    select: function() {
      return `<select class="${this.namespace}-select" name="${
        this.namespace
    ...
    option: function(content) {
      return `<option value="${this.options.tpl.optionValue.call(
        this
      )}">${content}</option>`;
    },
    items: function() {
    ...
    item: function(content) {
      return `<li class="${
        this.namespace
      }-item">${content}${this.options.tpl.itemRemove.call(this)}</li>`;
    },
    itemRemove: function() {
  ...
  build(data) {
    ...
    if (this.instance.options.buildFromHtml === true) {
      if ($options.length !== 0) {
        $.each($options, (i, n) => {
          const $li = $(
            this.instance.options.tpl.listItem.call(this.instance, n.text)
          );
          const $n = $(n);
      ...
      $.each(data, i => {
        const $li = $(
          this.instance.options.tpl.listItem.call(this.instance, data[i])
        );
      ...
      if ($options.length !== 0) {
        $.each($options, (i, n) => {
          const $n = $(n);
          const li = this.instance.getItem(
            'li',
            $list,
            this.instance.options.tpl.optionValue($n.data('selective_index'))
          );
  ...
  loadMore() {
    ...
    this.instance.$listWrap.on('scroll.selective', () => {
      if (pageMax > this.instance.page) {
        ...
        if (below === 0) {
          this.instance.options.query(
            this.instance,
            this.instance.$search.val(),
            ++this.instance.page
          );
        }
      }
  ...
  withDefaults(data) {
    if (data !== null) {
      $.each(data, i => {
        ...
        this.instance._options.select(
          this.instance.getItem(
            'option',
            this.instance.$select,
            this.instance.options.tpl.optionValue(data[i])
          )
        );
  ...
  remove(obj) {
    ...
    if (this.instance.options.buildFromHtml === true) {
      ...
      $li = this.instance.getItem(
        'li',
        this.instance.$list,
        this.instance.options.tpl.optionValue(obj.data('selective_index'))
      );
      if ($li !== undefined) {
...
class Selective {
  ...
  _show() {
    $$1(document).on('click.selective', e => {
      if (this.options.closeOnSelect === true) {
        if (
          $$1(e.target).closest(this.$triggerButton).length === 0 &&
  ...
  select($li) {
    ...
    if (this.options.buildFromHtml === true) {
      ...
      this._options.select(
        this.getItem('option', this.$select, this.options.tpl.optionValue(data))
      );
      this.itemAdd(data);
    }
  ...
  getItem(type, $list, index) {
    ...
    for (let i = 0; i < $items.length; i++) {
      if (
        this.options.tpl.optionValue($items.eq(i).data('selective_index')) ===
        index
      ) {
        position = i;
      }
